(function() {
    ...
    var food01_period = "<%= period %>";
    var food01_dif = 1;

    if (food01_period == "week") {
        food01_dif = 7;
    } else if (food01_period == "month") {
        var f1dt = new Date();
        f1dt.setDate(0);
        food01_dif = f1dt.getDate();
    }
    ...
    apiRequest("/sensor/food01-t1;food01-h1/<%= period %>")
        .success(function(data) {
            ...
            for (var sensor in data) {
                ...
                var i = 0;
                for (i = 0; i < l; i++) {
                    var dtime = new Date(Date.parse(data[sensor]["past"][i].time) + food01_dif * 24 * 3600000);
                    past_data.push({
                        x: dtime,
